...
class ProfilesModel extends ListModel {
    ...
    protected function getListQuery() {
        ...
        // Join over the user field 'modified_by'
        $query->select('`u`.name AS `real_name`, `u`.email');
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                $query->where('( a.id LIKE ' . $search . '  OR  u.name LIKE ' . $search . '  OR  u.email LIKE ' . $search . '  OR  a.preferred_name LIKE ' . $search . '  OR  a.home_group LIKE ' . $search . ' )');
            }
        }
