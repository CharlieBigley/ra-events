...
//        Factory::getApplication()->enqueueMessage($message, 'info');
    ...
    public function delete() {
        ...
        $sql = 'SELECT title, state FROM #__ra_events WHERE id=';
        foreach ($primary_keys as $id) {
            $item = $this->toolsHelper->getItem($sql . $id);
            $message = '"' . $item->title . '" ';

            if ($item->state == 0) {
                $this->toolsHelper->executeCommand('DELETE FROM #__ra_events WHERE id=' . $id);
                $message .= 'deleted';
            } else {
    ...
    public function extractBookings() {
        ...
        echo '<h4>You can copy this report and paste it into a CSV file, then import the details into a different Event</h4>';
...
//        Factory::getApplication()->enqueueMessage($this->task . ", from $from to $to ", 'warning');
    ...
    public function showBookings() {
        ...
        $sql .= 'p.preferred_name, e.group_code, s.title ';
