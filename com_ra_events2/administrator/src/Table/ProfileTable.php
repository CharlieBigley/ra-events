...
//XXX_CUSTOM_TABLE_FUNCTION
    ...
    private function writeProfile($profile_id, $user_id) {
        ...
        if ($profile_id == 0) {
            $app->enqueueMessage('Creating profile ' . $user_id . ' for ' . $this->preferred_name . ' ', 'info');
            ...
            $app->enqueueMessage('Updating profile ' . $profile_id . ' for ' . $this->preferred_name . ' ', 'info');
            $app->enqueueMessage($message, 'info');
            $query->set("updated = " . $db->quote($date))
                    ->set("updated_by = " . $db->quote($current_user));
            $query->update('#__ra_profiles')
                    ->where('id=' . $profile_id);
            $result = $db->setQuery($query)->execute();
        }
        return $result;
