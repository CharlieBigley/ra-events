...
// Find name of the User
        if ($this->item->id == 0) {
            $this->title = 'Hi ' . $bookingHelper->lookupPreferredname($this->user->id);
            $sql .= 'booking_info ';
...
//        die;
        ...
        $this->intro .= '<b>Contact</b> ' . $event->name . '<br> ';
//        $this->intro .= '<span class="icon-envelope" aria-hidden="true"></span>';
        ...
        if ($this->item->id == 0) {
            ...
            $this->title .= $bookingHelper->lookupPreferredname($event->user_id);
