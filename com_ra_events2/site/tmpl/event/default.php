...
if (is_null($this->item->api_site_id)) {
    ...
    $sql = 'SELECT colour FROM #__ra_api_sites WHERE id=' . $this->item->api_site_id;
    $colour = $this->toolsHelper->getValue($sql);
    echo '<div style="background: ' . $colour . '; ">';
}
...
echo $bookingHelper->showBookings($this->item->bookable, $this->item->id, $callback);
